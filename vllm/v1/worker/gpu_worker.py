...
class Worker(WorkerBase):
    ...
    def _measure_kv_cache_blocks(self) -> int:
        ...
        The profiled activation peak is kept free too, except for the part
        torch's caching allocator already holds in its pool; graph capture
        empties that pool, so it is not reliably resident at this point.
        """
        torch.accelerator.synchronize()
        gc.collect()
        free_memory, _ = torch.accelerator.get_memory_info(self.device)
        stats = torch.accelerator.memory_stats(self.device)
        cached_pool = stats.get("reserved_bytes.all.current", 0) - stats.get(
        ...
        num_blocks = measure_kv_cache_blocks(
            ...
            + max(0, kv_cache.reserved_headroom_bytes - cached_pool),
        )
        num_blocks = (
